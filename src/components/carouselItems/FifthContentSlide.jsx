...
function FifthContentSlide(props) {
  ...
  useEffect(() => {
    setSlideSettings({ ...slideSettings, titleValue: t('slide-title'), descriptionValue: t('slide-desc') })
  }, [t])
  ...
  return (
    <div className="slide-wrapper">
      <div
        className={`slide_container content-slide ${alignment === "center" ? "text-center" : alignment === "right" ? "text-end" : "text-start"
      ...
      >
        ...
        <div className="slide-content text-white">
          {slideSettings?.type !== "imageOnly" && (
            <div className="slide__text-wrapper">
              {slideCounterInfo?.status && (
                <span
                  className={`slider__counter--box ${alignment === "center" ? "mx-auto" : alignment === "right" ? "ms-auto" : ""}`}
              ...
              {(slideSettings.title &&
                (slideSettings?.type === "textAndImage" ||
                  slideSettings?.type === "textOnly")) && (
                  <div className={`title title-font-${textSize}`} style={{ color: colorPalate.third, fontFamily: fontFamily }}>
                    <StyledSentence sentence={slideSettings.titleValue} color={colorPalate.second} />
                  </div>
                )}
              {(slideSettings.description &&
                (slideSettings?.type === "textAndImage" ||
                  slideSettings?.type === "textOnly")) && (
                  <div className={`title description-font-${textSize}`} style={{ color: colorPalate.third, fontFamily: fontFamily2 }}>
                    {slideSettings.descriptionValue}</div>
                )}
            </div>
          )}
          {(slideSettings?.type === "imageOnly" ||
          ...
          brandingType === "personal" && !personalInfo?.introOutroOnly && (
            <div className="contentSlide__bottom-data">
              <div className="branding_data d-flex align-items-center gap-2">
                ...
                <div>
                  {
                    personalInfo?.isName && <h6 className="m-0"
                      style={{ color: colorPalate.third, fontFamily: fontFamily }}>{personalInfo?.name}</h6>
                  }
                  {
                    personalInfo?.isHandle && <p className="m-0 fs-xs"
                      style={{ color: colorPalate.third, fontFamily: fontFamily }}>{personalInfo?.handle}</p>
                  }
                </div>
              </div>
            </div>
      ...
      {
        props.download && <div className="slide_individual_settings">
          <div className="top__buttons-wrapper">
            <Button
              ...
              name={<><FeatherIcon icon="arrow-left" size={14} /> {t('reorder')}</>}
            />
            ...
            <Button
              ...
              name={<>{t('duplicate')} <FeatherIcon icon="plus" size={14} /></>}
            />
            ...
            <Button
              ...
              name={<>{t('reorder')} <FeatherIcon icon="arrow-right" size={14} /></>}
            />
          </div>
          <div className="configure-title">{t('fifth-slide-setting')}</div>
          <div className="configure-container">
            ...
            <div className="btn-group w-100">
              <Input
                type="radio"
                className="btn-check"
                name="fislideSettings"
                id="fitextOnly"
                value="textOnly"
                autoComplete="true"
                checked={slideSettings.type === "textOnly"}
                handleChange={(e) =>
                  setSlideSettings({ ...slideSettings, type: e.target.value })
                }
              />
              <label className="btn btn__radio-check" htmlFor="fitextOnly">
                {t('textOnly')}
              </label>
              ...
              <Input
                ...
                value="textAndImage"
                autoComplete="true"
                checked={slideSettings.type === "textAndImage"}
                handleChange={(e) =>
                  setSlideSettings({ ...slideSettings, type: e.target.value })
                }
              />
              <label className="btn btn__radio-check" htmlFor="fitextAndImage">
                {t('textandimage')}
              </label>
              ...
              <Input
                ...
                value="imageOnly"
                autoComplete="true"
                checked={slideSettings.type === "imageOnly"}
                handleChange={(e) =>
                  setSlideSettings({ ...slideSettings, type: e.target.value })
                }
              />
              <label className="btn btn__radio-check" htmlFor="fiimageOnly">
                {t('imageOnly')}
              </label>
            </div>
          </div>
              ...
              <div className="configure-container">
                <div className="form-check form-switch">
                  <Input
                    ...
                    name="title"
                    checked={slideSettings.title === true}
                    handleChange={(e) =>
                      setSlideSettings({
                        ...slideSettings,
              ...
              <div className="configure-container">
                ...
                <form onSubmit={fetchImages}>
                  <Row className="gy-3 gx-2">
                    ...
                    {
                      images?.length > 0 && (<Col xs={12}>
                        <div className="searched__images-result">
                          {
                            images && images.length > 0 ? images?.map((image, index) => {
                              return <img
                                src={image.src.medium}
                                onClick={() => fetchAndConvertToBase64(image.src.landscape)}
                                alt=""
                                className="img-fluid"
                              />
                            }) : ""
...
export default FifthContentSlide;
